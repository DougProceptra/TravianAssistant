  ...
  private validateBuilding(
    ...
    for (const [level, expected] of Object.entries(expectedValues)) {
      ...
      if (calculated.wood !== exp.wood) {
        this.errors.push(
          `❌ ${buildingName} L${level} Wood: Expected ${exp.wood}, got ${calculated.wood} (diff: ${calculated.wood - exp.wood})`
        );
      }
      if (calculated.clay !== exp.clay) {
        this.errors.push(
          `❌ ${buildingName} L${level} Clay: Expected ${exp.clay}, got ${calculated.clay} (diff: ${calculated.clay - exp.clay})`
        );
      }
      if (calculated.iron !== exp.iron) {
        this.errors.push(
          `❌ ${buildingName} L${level} Iron: Expected ${exp.iron}, got ${calculated.iron} (diff: ${calculated.iron - exp.iron})`
        );
      }
      if (calculated.crop !== exp.crop) {
        this.errors.push(
          `❌ ${buildingName} L${level} Crop: Expected ${exp.crop}, got ${calculated.crop} (diff: ${calculated.crop - exp.crop})`
        );
      }
    }
  ...
  validateBuildTimes(): void {
    ...
    for (const [level, expectedTime] of Object.entries(HERO_MANSION_BUILD_TIMES)) {
      ...
      if (storedTime !== expectedTime) {
        this.errors.push(
          `❌ Hero Mansion L${level} Build Time: Expected ${expectedTime}s, got ${storedTime}s`
        );
      }
    }
